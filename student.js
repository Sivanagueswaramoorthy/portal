...
// 🛑 ANTI-BREAK ESCAPER (Prevents HTML from breaking on special characters)
const esc = (str) => {
    if (!str) return '--';
...
window.onload = async () => {
    try {
        ...
        // 🛑 FIXED ROLE CHECK: Allows ALL admins (Staff & Placement) to access the portal
...
// --- COLLEGE PLACEMENT STATS & EXCEL EXPORT ---
...
function executeExcelExport() {
    ...
    // Salary filter is complex because it's not strictly loaded in `allGlobalPlacements` array, 
    // so we will export all matching criteria!
