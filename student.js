...
window.onload = async () => {
    ...
    try {
        ...
        let loggedInName = data.profile.full_name; let loggedInEmail = data.profile.email; let loggedInPic = data.picture || getAvatar(loggedInName);
...
function populateDashboard(p, img, courses, skills, semGpas) {
    ...
    if(skills && skills.length > 0) {
        ...
        document.getElementById('skills-container').innerHTML = skills.map(s => {
            ...
            return `<div class="skill-card"><div class="skill-header flex-between"><div class="skill-title"><span>${s.skill_name}</span></div></div><div class="badge" style="margin-bottom: 12px;">${s.category || 'General'}</div><div class="progress-track"><div class="progress-fill" style="width: ${pct}%;"></div></div><div class="skill-footer flex-between"><div class="flex-center"><i class="fa-solid fa-layer-group" style="color: var(--primary);"></i> <span style="color: var(--text-main); font-weight:800;">${s.completed_levels}</span> / <span>${s.total_levels}</span> Lvl</div><div style="color: var(--primary); font-weight: 800;">${pct}%</div></div></div>`;
        }).join('');
    } else { 
...
function populateGlobalPlacement(gStats, gDrives) {
    ...
    if (gDrives && gDrives.length > 0) {
        drvBody.innerHTML = gDrives.map(d => `<tr><td style="font-weight: 700; color: var(--text-main);">${d.company}</td><td>${d.role}</td><td style="font-family: monospace;">${d.appeared}</td><td><span class="badge badge-success">${d.selected}</span></td><td style="font-weight: 700; color: var(--primary);">${d.ctc}</td></tr>`).join('');
    } else { drvBody.innerHTML = `<tr><td colspan="5" style="text-align:center; padding: 30px; color:var(--text-muted);">No campus drives recorded.</td></tr>`; }
}
...
function populatePersonalPlacement(pProfile, pApps) {
    ...
    if (pApps && pApps.length > 0) {
        appBody.innerHTML = pApps.map(a => {
            ...
            return `<tr><td style="font-weight: 700; color: var(--text-main);">${a.company}</td><td style="color: var(--text-muted);">${a.role}</td><td>${a.date_applied}</td><td><span class="badge ${bClass}">${a.status}</span></td></tr>`;
        }).join('');
