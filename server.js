...
// --- DATABASE AUTO-INITIALIZER & MIGRATION ---
(async function initializeDatabase() {
    try {
        ...
        console.log("✅ Database Verified: Universal Deletion Supported.");
...
async function verifyAdmin(reqBody) {
    ...
    if (email !== 'sivanagu7771@gmail.com' && email !== 'admin@gmail.com') throw new Error("Unauthorized Email: " + email); 
    return true;
}
...
app.post('/api/admin/student-data', async (req, res) => { 
    try { 
        ...
        const rawEmail = req.body.targetEmail || req.body.email; 
        if(!rawEmail) throw new Error("Email not provided");
...
// 🛑 COMPLETE CASCADING USER DELETE (FIXED FOR BOTH PORTALS)
app.post('/api/admin/delete-student', async (req, res) => {
    try {
        await verifyAdmin(req.body);
        
        // 🛑 Accepts BOTH variable names (targetEmail from new portal, email from old admin portal)
        const rawEmail = req.body.targetEmail || req.body.email; 
        if(!rawEmail) return res.json({ success: false, message: "No email provided to delete." });
        
        const targetEmail = rawEmail.toLowerCase();
        
        // Wipe from all tables to ensure zero orphaned records
        ...
        console.error("DB Error Delete Student:", e);
        res.json({ success: false, message: e.message });
    }
});

// Alias for old portal just in case it uses a different route name
app.post('/api/admin/delete-user', async (req, res) => {
    // Reroute to delete-student logic
    req.url = '/api/admin/delete-student';
    app.handle(req, res);
});
...
app.post('/api/admin/add-app', async (req, res) => { 
    try { 
        await verifyAdmin(req.body); 
        const rawEmail = req.body.targetEmail || req.body.email;
        await promisePool.query("INSERT INTO placement_apps (student_email, company, role, date_applied, status) VALUES (?, ?, ?, ?, ?)", [rawEmail.toLowerCase(), req.body.company, req.body.role, req.body.date_applied, req.body.status]); 
        res.json({ success: true }); 
    } catch (e) { res.json({ success: false, message: e.message }); } 
});
...
app.post('/api/admin/update-field', async (req, res) => { 
    try { 
        await verifyAdmin(req.body); 
        const rawEmail = req.body.targetEmail || req.body.email;
        await promisePool.query(`UPDATE student_profile SET ${req.body.field} = ? WHERE LOWER(email) = LOWER(?)`, [req.body.value, rawEmail.toLowerCase()]); 
        res.json({ success: true }); 
    } catch (e) { res.json({ success: false, message: e.message }); } 
});
...
app.post('/api/admin/update-placement-profile', async (req, res) => { 
    try { 
        await verifyAdmin(req.body); 
        const rawEmail = req.body.targetEmail || req.body.email;
        await promisePool.query(`INSERT IGNORE INTO placement_student_profile (student_email) VALUES (?)`, [rawEmail.toLowerCase()]); 
        await promisePool.query(`UPDATE placement_student_profile SET ${req.body.field} = ? WHERE student_email = ?`, [req.body.value, rawEmail.toLowerCase()]); 
        res.json({ success: true }); 
    } catch (e) { res.json({ success: false, message: e.message }); } 
});
...
app.post('/api/announcements/list', async (req, res) => {
    try {
        let incomingToken = req.body.token || "";
        if (typeof incomingToken === 'string') incomingToken = incomingToken.replace(/['"]+/g, '');
        if (incomingToken === 'custom_admin_token_pc123') { const [rows] = await promisePool.query("SELECT * FROM announcements ORDER BY date_posted DESC"); return res.json({ success: true, announcements: rows }); }
        ...
        if (email === 'sivanagu7771@gmail.com' || email === 'admin@gmail.com') { const [rows] = await promisePool.query("SELECT * FROM announcements ORDER BY date_posted DESC"); return res.json({ success: true, announcements: rows }); }
...
app.post('/api/drives/active-list', async (req, res) => {
    try {
        let incomingToken = req.body.token || "";
        if (typeof incomingToken === 'string') incomingToken = incomingToken.replace(/['"]+/g, '');
        if (incomingToken === 'custom_admin_token_pc123') { const [rows] = await promisePool.query("SELECT * FROM active_drives ORDER BY id DESC"); return res.json({ success: true, drives: rows }); }
        const ticket = await googleClient.verifyIdToken({ idToken: req.body.token, audience: CLIENT_ID }); const email = ticket.getPayload().email.toLowerCase();
        if(email === 'sivanagu7771@gmail.com' || email === 'admin@gmail.com') { const [rows] = await promisePool.query("SELECT * FROM active_drives ORDER BY id DESC"); return res.json({ success: true, drives: rows }); }
...
app.post('/api/admin/delete-active-drive', async (req, res) => { 
    try { 
        ...
        if (drive.length > 0) {
            ...
            await promisePool.query("UPDATE placement_student_profile SET offer_company = '--', offer_role = '--', offer_ctc = '--', status = 'Unplaced' WHERE offer_company = ? AND offer_role = ?", [comp, role]);
        }
...
app.post('/api/admin/mark-placed', async (req, res) => {
    try {
        ...
        if(req.body.status === 'Placed' || req.body.status === 'Selected') {
            ...
            if(app.length > 0) {
                await promisePool.query(`INSERT INTO placement_student_profile (student_email, offer_company, offer_role, offer_ctc, status) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE offer_company = ?, offer_role = ?, offer_ctc = ?, status = ?`, [app[0].student_email, app[0].company, app[0].role, req.body.package, req.body.status, app[0].company, app[0].role, req.body.package, req.body.status]);
            }
        }
        res.json({ success: true });
    } catch (e) { res.json({ success: false, message: e.message }); }
