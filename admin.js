...
let loggedInName = ""; let loggedInEmail = ""; let loggedInPic = ""; 
let currentGlobalStats = null; let currentGlobalDrives = [];
...
function updateSidebarNav() {
    const adminGlobals = document.querySelectorAll('.admin-global'); const studentNavs = document.querySelectorAll('.student-nav'); const backBtn = document.getElementById('nav-back');
    if (targetStudentEmail === "") {
        adminGlobals.forEach(el => el.style.display = 'flex'); studentNavs.forEach(el => el.style.display = 'none'); backBtn.style.display = 'none';
    } else {
        adminGlobals.forEach(el => el.style.display = 'none'); studentNavs.forEach(el => el.style.display = 'flex'); backBtn.style.display = 'flex';
    }
}

function switchTab(tabId, element) {
    document.querySelectorAll('.nav-item').forEach(nav => nav.classList.remove('active')); if(element) element.classList.add('active');
    document.querySelectorAll('.view-section').forEach(view => view.classList.remove('active')); document.getElementById('view-' + tabId).classList.add('active');
...
window.onload = async () => {
    google.accounts.id.initialize({ client_id: "159246343111-o9bv4lgk1hmmvdkef0qnq0ih9qefjhmj.apps.googleusercontent.com", callback: () => {} });
    try {
...
function renderDirectoryTable(students) {
    ...
    if(students.length === 0) return tbody.innerHTML = `<tr><td colspan="5" style="text-align: center; padding: 40px; color: var(--text-muted);">No students found.</td></tr>`;
...
async function loadStudentData(email) {
    ...
    if (data.success) { 
        ...
        if(!activeTab || activeTab.id === 'nav-dir' || activeTab.id === 'nav-col-place') switchTab('dashboard', document.getElementById('nav-dash'));
    }
}
...
function renderChart(courses, semGpas) {
    ...
    if (courses && courses.length > 0) {
        ...
        courses.forEach(c => {
            ...
            let pts = 0;
            if(c.grade.includes('O')) pts = 10; else if(c.grade === 'A+') pts = 9; else if(c.grade === 'A') pts = 8; else if(c.grade === 'B+') pts = 7; else if(c.grade === 'B') pts = 6; else if(c.grade === 'C') pts = 5;
...
function populateDashboard(p, img, courses, skills, semGpas) {
    ...
    document.getElementById('val-cgpa').innerText = parseFloat(p.cgpa).toFixed(2); document.getElementById('val-sgpa').innerText = parseFloat(p.sgpa || 0).toFixed(2);
        ...
        document.getElementById('skills-container').innerHTML = skills.map(s => {
            ...
            <div class="skill-card" id="card-sk-${s.id}">
                ...
                <div class="skill-footer flex-between">
                    <div class="flex-center"><i class="fa-solid fa-layer-group" style="color: var(--primary);"></i> <span id="sk-c-${s.id}" style="color: var(--text-main); font-weight:800;">${s.completed_levels}</span> / <span id="sk-t-${s.id}">${s.total_levels}</span> Lvl</div>
            ...
            <div class="sem-card">
                <div class="sem-header">
                    <div class="flex-center">
                        ...
                        <div class="sem-gpa-badge"><span class="lbl">GPA</span><div id="wrap-semgpa-${sem}" class="flex-center"><span id="val-semgpa-${sem}" class="val">${semGpaVal}</span><i class="fa-solid fa-pen admin-table-edit" style="padding: 2px;" onclick="openSemGpaEdit(${sem})"></i></div></div>
                    </div>
...
function populateGlobalPlacement(gStats, gDrives) {
    gStats = gStats || {};
    document.getElementById('val-g-total').innerText = gStats.total_placed || '0'; document.getElementById('val-g-ongoing').innerText = gStats.ongoing_drives || '0';
    document.getElementById('val-g-highest').innerText = gStats.highest_ctc || '0'; document.getElementById('val-g-avg').innerText = gStats.avg_ctc || '0';
    const drvBody = document.getElementById('global-drives-tbody');
    if (gDrives && gDrives.length > 0) {
        drvBody.innerHTML = gDrives.map(d => `
        <tr id="row-drv-${d.id}">
            <td style="font-weight: 700; color: var(--text-main);">${d.company}</td><td>${d.role}</td><td style="font-family: monospace;">${d.appeared}</td>
            <td><span class="badge badge-success">${d.selected}</span></td><td style="font-weight: 700; color: var(--primary);">${d.ctc}</td>
            <td style="text-align:right; padding-right: 24px; white-space:nowrap;">
...
function populatePersonalPlacement(pProfile, pApps) {
    ...
    document.getElementById('val-p-role').innerText = prf.offer_role || '--'; document.getElementById('val-p-comp').innerText = prf.offer_company || '--';
    document.getElementById('val-p-ctc').innerText = prf.offer_ctc || '--'; document.getElementById('val-p-status').innerText = prf.status || 'Unplaced';
    document.getElementById('val-p-assess').innerText = prf.assessments || '0'; document.getElementById('val-p-int').innerText = prf.interviews || '0'; document.getElementById('val-p-off').innerText = prf.offers || '0';
    ...
    if (pApps && pApps.length > 0) {
        appBody.innerHTML = pApps.map(a => {
            ...
            return `
            <tr id="row-app-${a.id}">
                <td style="font-weight: 700; color: var(--text-main);">${a.company}</td><td style="color: var(--text-muted);">${a.role}</td>
                <td>${a.date_applied}</td><td><span class="badge ${bClass}">${a.status}</span></td>
                <td style="text-align:right; padding-right: 24px; white-space:nowrap;">
...
async function submitNewStudent() {
    const email = document.getElementById('new-email').value; const full_name = document.getElementById('new-name').value;
    const roll_no = document.getElementById('new-roll').value; const department = document.getElementById('new-dept').value;
    await fetch(`${BASE_URL}/api/admin/add-student`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminToken: globalToken, email: email, full_name: full_name, roll_no: roll_no, department: department }) });
    closeModal('add-modal'); fetchDirectory();
}
async function deleteStudent(email) {
    if(!confirm("Delete this student and ALL records permanently?")) return;
    await fetch(`${BASE_URL}/api/admin/delete-student`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminToken: globalToken, email: email }) }); fetchDirectory(); 
}
async function submitNewSkill() {
    const skill_name = document.getElementById('sk-name').value; const category = document.getElementById('sk-cat').value;
    const completed_levels = document.getElementById('sk-comp').value; const total_levels = document.getElementById('sk-tot').value;
    await fetch(`${BASE_URL}/api/admin/add-skill`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminToken: globalToken, targetEmail: targetStudentEmail, skill_name: skill_name, category: category, completed_levels: completed_levels, total_levels: total_levels }) });
    closeModal('add-skill-modal'); loadStudentData(targetStudentEmail); 
}
async function submitNewCourse() {
    const semester = document.getElementById('crs-sem').value; const course_name = document.getElementById('crs-name').value;
    const marks = document.getElementById('crs-mark').value; const grade = document.getElementById('crs-grade').value;
    await fetch(`${BASE_URL}/api/admin/add-course`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminToken: globalToken, targetEmail: targetStudentEmail, semester: semester, course_name: course_name, marks: marks, grade: grade }) });
    closeModal('add-course-modal'); loadStudentData(targetStudentEmail); 
}
async function deleteSkill(id) { if(!confirm("Delete this skill?")) return; await fetch(`${BASE_URL}/api/admin/delete-skill`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminToken: globalToken, id: id }) }); loadStudentData(targetStudentEmail); }
async function deleteCourse(id) { if(!confirm("Delete this course?")) return; await fetch(`${BASE_URL}/api/admin/delete-course`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminToken: globalToken, id: id }) }); loadStudentData(targetStudentEmail); }

async function submitNewDrive() {
    const company = document.getElementById('drv-comp').value; const role = document.getElementById('drv-role').value;
    const appeared = document.getElementById('drv-app').value; const selected = document.getElementById('drv-sel').value; const ctc = document.getElementById('drv-ctc').value;
    await fetch(`${BASE_URL}/api/admin/add-drive`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminToken: globalToken, company: company, role: role, appeared: appeared, selected: selected, ctc: ctc }) });
    closeModal('add-drive-modal'); window.location.reload(); 
}
async function submitNewApp() {
    const company = document.getElementById('app-comp').value; const role = document.getElementById('app-role').value;
    const date_applied = document.getElementById('app-date').value; const status = document.getElementById('app-stat').value;
    await fetch(`${BASE_URL}/api/admin/add-app`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminToken: globalToken, targetEmail: targetStudentEmail, company: company, role: role, date_applied: date_applied, status: status }) });
    closeModal('add-app-modal'); loadStudentData(targetStudentEmail); 
}
async function deleteDrive(id) { if(!confirm("Delete this drive?")) return; await fetch(`${BASE_URL}/api/admin/delete-drive`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminToken: globalToken, id: id }) }); window.location.reload(); }
async function deleteApp(id) { if(!confirm("Delete application?")) return; await fetch(`${BASE_URL}/api/admin/delete-app`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminToken: globalToken, id: id }) }); loadStudentData(targetStudentEmail); }
...
function cancelProfileEdit(spanId, field, width) { document.getElementById(`in-${spanId}`).parentElement.parentElement.innerHTML = `<span id="${spanId}" style="${field==='email'?'word-break:break-all;':''}">${originalValues[spanId]}</span><i class="fa-solid fa-pen admin-table-edit" onclick="openProfileEdit('${field}', '${spanId}', '${width}')"></i>`; }
async function saveProfileEdit(field, spanId, width) {
    const val = document.getElementById(`in-${spanId}`).value; document.getElementById(`in-${spanId}`).parentElement.parentElement.innerHTML = `<i class="fa-solid fa-spinner fa-spin" style="color: var(--primary);"></i>`;
    const req = await fetch(`${BASE_URL}/api/admin/update-field`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminToken: globalToken, targetEmail: targetStudentEmail, field: field, value: val }) });
...
function openSemGpaEdit(sem) {
    const span = document.getElementById(`val-semgpa-${sem}`); const val = span.innerText === '--' ? '' : span.innerText;
    span.parentElement.innerHTML = `
...
async function saveSemGpa(sem) {
    ...
    await fetch(`${BASE_URL}/api/admin/update-sem-gpa`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ adminToken: globalToken, targetEmail: targetStudentEmail, semester: sem, gpa: val }) });
...
async function saveGlobalStat(field, spanId, width) {
    ...
    await fetch(`${BASE_URL}/api/admin/update-global-stat`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminToken: globalToken, field: field, value: val }) });
...
function editDriveRow(id) {
    ...
    const comp = tr.children[0].innerText; const role = tr.children[1].innerText;
    const app = tr.children[2].innerText; const sel = tr.children[3].innerText; const ctc = tr.children[4].innerText;
...
function editAppRow(id) {
    ...
    const comp = tr.children[0].innerText; const role = tr.children[1].innerText;
    const date = tr.children[2].innerText; const stat = tr.children[3].innerText;
...
function editSkillCard(id) {
    ...
    const name = document.getElementById(`sk-n-${id}`).innerText; const comp = document.getElementById(`sk-c-${id}`).innerText; const total = document.getElementById(`sk-t-${id}`).innerText;
    ...
    card.innerHTML = `
        ...
        <div class="flex-center" style="margin-bottom: 20px;">
            <span style="font-size:0.7rem; font-weight:700; color:var(--text-muted);">CMP:</span><input type="number" id="edit-sk-c-${id}" class="inline-input" style="width: 60px;" value="${comp}">
            <span style="font-size:0.7rem; font-weight:700; color:var(--text-muted); margin-left:12px;">TOT:</span><input type="number" id="edit-sk-t-${id}" class="inline-input" style="width: 60px;" value="${total}">
        </div>
...
async function saveSkillCard(id) {
    ...
    await Promise.all(updates.map(u => fetch(`${BASE_URL}/api/admin/update-skill`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminToken: globalToken, id: id, field: u.field, value: u.value }) })));
    loadStudentData(targetStudentEmail);
}